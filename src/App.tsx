...
const App: React.FC = () => {
  ...
  const [currentUser] = useState('sillydoge');
  const [currentHost] = useState('terminal');
  const [currentPath, setCurrentPath] = useState('~');
  const [commandHistory, setCommandHistory] = useState<string[]>([]);
  const [historyIndex, setHistoryIndex] = useState(-1);
  const [showInstructions, setShowInstructions] = useState(true);
  const [animationPhase, setAnimationPhase] = useState<'linux' | 'sillydoge' | 'crash' | 'loading' | 'complete'>('linux');
  ...
  useEffect(() => {
    if (animationPhase === 'linux') {
      ...
      // SillyDoge显示动画
  useEffect(() => {
          if (animationPhase === 'sillydoge') {
              const sillyDogeText = 'SillyDoge';
      let currentIndex = 0;
      const interval = setInterval(() => {
    ...
    switch (command) {
      ...
      case 'fortune':
        const fortunes = [
          ...
          'Good code means more bugs! This is SillyDoge philosophy!',
          'What programmers hate most: writing documentation! So we don\'t!',
          'The best code is no code! That\'s why we call it SillyDoge!',
          'Debugging is twice as hard as writing code! So we don\'t debug!',
          'Time is money, my friend! But in the SillyDoge world, time is happiness!',
          ...
          'In SillyDoge we trust.',
          'Reboot your brain, not just your computer.',
          'Error: SillyDoge has stopped working.',
      ...
      <div className={`App boot-animation-screen ${crashEffect ? 'crash-effect' : ''}`}>
        ...
        {animationPhase === 'loading' && (
          <div className="loading-animation">
            <div className="loading-text">SillyDoge</div>
    ...
    <div className={`App ${glitchEffect ? 'glitch' : ''}`}>
      ...
      <div className="terminal-container">
        <div className="terminal-header">
          ...
          <div className="terminal-title">SillyDoge Linux Terminal v1.0.0</div>
        </div>
          ...
          <div className="terminal-content">
            ...
            {showInstructions && (
              <div className="instructions">
                <div className="instructions-title">=== SillyDoge TERMINAL GUIDE ===</div>
                <div>This is the SillyDoge terminal interface. You can input any Linux command, and I will respond according to SillyDoge rules.</div>
